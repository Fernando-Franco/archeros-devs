...
const refactoredResgate = function(date){
    var givenDate = moment(date);
    var dayOfWeek = givenDate.day();
    ...
    if (isSaturday) { // Sabado
        givenDate.add(2, 'days');
    } else
    if (isSunday) { // Domingo
        givenDate.add(1, 'days');
    } else
    if(isHoliday(givenDate) && !isFriday){ // Feriado na semana
        givenDate.add(1, 'days');
    } else
    if(isHoliday(givenDate) && isFriday){ // Feriado na sexta
        givenDate.add(3, 'days');
    }

    return moment(givenDate).format('YYYY-MM-DD');
}

module.exports = { ehDiaUtil, refactoredResgate };